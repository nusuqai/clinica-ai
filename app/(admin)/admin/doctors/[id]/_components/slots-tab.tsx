...
import { toggleSlotBlockedAction } from "@/server/actions/admin";
...
export default function SlotsTab({ doctorId, slots }: SlotsTabProps) {
  ...
  // Group all slots by date first, then apply filter per-group
  ...
  // All date keys except the first start collapsed
  const [collapsed, setCollapsed] = useState<Set<string>>(
    () => new Set(Object.keys(grouped).sort().slice(1))
  );

  // Filtered counts for the summary bar
      ...
      {slots.length === 0 ? (
        ...
        <div className="space-y-2">
          {dateKeys.map((dateKey) => {
            ...
            const filteredSlots = filter === "all" ? daySlots : daySlots.filter((s) => slotStatus(s) === filter);
                          ...
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            {isBooked && (
                              <div className="flex items-center gap-2 min-w-0">
                                <AppointmentStatusBadge status={slot.appointment!.status as AppointmentStatus} />
