...
const BOOKING_FLOW =
  "عند رغبة المستخدم بالحجز: لا تسأله عن تاريخ محدد مباشرة ولا تفترض مواعيد. أولاً استخدم get_doctor_working_hours لعرض أيام وساعات عمل الطبيب والفرع الذي يداوم فيه كل يوم، دعه يختار يوماً يعمل فيه الطبيب فعلاً، ثم استخدم get_doctor_availability لعرض ما هو متاح في ذلك اليوم. لاحظ نظام الجدولة (mode): إن كان slot فاعرض الفترات (slots) بأوقاتها ليختار واحدة واحجز بـ book_appointment. وإن كان order (نظام الدور) فلا توجد أوقات ثابتة؛ أخبر المستخدم بعدد الأماكن المتبقية ورقم دوره القادم، واحجز بـ book_order_appointment (بدون slotId) — سيحصل على رقم دور، ومع تفعيل التتبّع يمكنه معرفة الدور الجاري الآن ومدة الانتظار التقديرية. بعد إتمام الحجز أو إعادة الجدولة، اعرض للمستخدم كل تفاصيل الموعد كما أعادتها الأداة تماماً (اسم الطبيب، الفرع، التاريخ، والوقت أو رقم الدور، والسعر إن وُجد) للتأكيد، واعتمد على قيم الأداة لا على ذاكرتك حتى ينتبه المستخدم لأي خطأ.";
